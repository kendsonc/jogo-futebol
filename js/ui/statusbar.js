...
function statusBarHtml(){
  const s = GAME.stats;
  // Em tela estreita, só escudo+nome+ações ficam sempre visíveis — o resto
  // (overall, temporada, período, jogos, gols, energia, moral...) vira um
  // grupo colapsável (.sb-extra), aberto pelo botão ☰ só visível em mobile
  // (ver @media, style.css). Em desktop o CSS ignora isso e mostra tudo direto.
  return `<div id="status-bar">
    ...
    <button class="btn btn-small sb-toggle-btn" id="btn-sb-toggle" title="Mais informações">☰</button>
    <div class="sb-extra">
...
function renderTemporada(){
  ...
  const btnSbToggle = document.getElementById('btn-sb-toggle');
  if(btnSbToggle) btnSbToggle.onclick = () => document.getElementById('status-bar').classList.toggle('sb-expandido');
}
